...
public class HospitalManagementServlet extends HttpServlet {
	...
	private void bookAppointment(HttpServletRequest request, HttpServletResponse response) throws IOException, SQLException {
		...
		Appointment appointment=new Appointment( doctorfirstName,doctorlastName,  specilization,  appointment_date, Location, Timeslot,  description,  Mobile,  Email);
		
		if(DBConnection.Appointment(appointment))
		{
			String to=Email;
	...
	private void listStudents(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		...
		ArrayList<Patient> details = (ArrayList<Patient>) DBConnection.Details(email);
		request.setAttribute("profile", details);
		
		
		String mail=null;
		ArrayList<Patient> profile=(ArrayList<Patient>) request.getAttribute("profile");
		
		 for(Patient std: profile)
	 		{
	 			 mail=std.getEmail();
...
Patient patientrecord=new Patient(access_level,email,password);
    loginConnection=DBConnection.Login(patientrecord.getAccess_level(),patientrecord.getEmail(),patientrecord.getPassword());
    out.println(loginConnection);
		}
		else
		{
			//doctorDetails doctorrecord=new doctorDetails(access_level,email,password);
			loginConnection=DBConnection.Login(access_level,email,password);
		}
	...
	private void addPatientdetails(HttpServletRequest request, HttpServletResponse response) throws SQLException, IOException {
		...
		Patient patientrecord=new Patient(patientName,Gender,DOB, Age,Mobile, Email,Password, maritalstatus);
		
		if(DBConnection.addPatientDetails(patientrecord))
		{
		response.sendRedirect("login.jsp");
	...
	private void addDoctordetails(HttpServletRequest request, HttpServletResponse response) throws SQLException, IOException, ServletException {
		...
		Doctor doctorrecord=new Doctor(FirstName,LastName,Gender,DOB,Mobile,Email,Password,Qualification,speciality,Experience);
		
	
		
		if(DBConnection.addDoctorDetails(doctorrecord))
		{
